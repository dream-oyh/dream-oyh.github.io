...
- 大端：高位字节存低位地址，符合人眼阅读顺序，比如：`0x123456` 存放在数组为： `{0x12, 0x34, 0x56}`
- 小端：高位字节存高位地址，符合计算机读取顺序，比如：`0x123456`，存放在数组为： `{0x56, 0x34, 0x12}`

## 命名规范

- 变量命名
  - 有具体含义的变量统一小驼峰命名
  - 循环等局部变量采用`i` `j` `k`定义即可
  - 由`static`定义的静态变量由`s_`开头，再接小驼峰，如：`s_sendBuffer`
  - 全局变量定义为`g_`，如`g_status`
  - `bool`变量统一由`is`开头
  - 共同约定好变量缩写：
    - `temp` -> temporary
    - `incre` -> increment
...
- 函数命名
  - 统一用大驼峰命名
  - 为了便于区分模块，同一模块下的函数命名可以由共同的全大写前缀开头，如：`ADDR_AddSnap()`
  - 对于由`static`定义的静态变量，需要定义`getter`和`setter`接口方便外部调用和修改；
    - `getter`函数：`模块名_Get变量名`
    - `setter`函数：`模块名_Set变量名`
  - 返回值为`bool`类型的函数统一以`Is`开头
- 结构体命名

  - 大驼峰命名
  - 结构体类型命名以`S_`开头，声明这是一个结构体标识符
  - 结构体定义统一采用`typedef`
...
- 基本数据类型就不说了。
- `stdint`标准库提供了`uint8_t` `uint16_t`等数据类型，用于定义无符号 8 位、16 位等整型数据，这在嵌入式中运用甚广。
- 在数字前加上`0x`表示该值为 16 进制；`0b`表示二进制

### 位运算

- C 中`&`和`|`提供了按位与/或计算
- 还提供了`>>`和`<<`的右移和左移运算
- 一字节是 8bit，也就是有 8 位数据；`0x00`表示一个字节。
- 对于一个`uint8_t`数据，一共有 8 位，能表示的最大值为 225；其余同理
- 我们能通过位运算/或运算/左移运算/右移运算实现位的精确操作

  - 将两个`uint8_t`数据拼成一个`uint16_t`数据（大端序拼接）
    ```C
    uint16_t merge(uint8_t high, uint8_t low){
      uint16_t result = 0;
      return result | (high << 8) | low;
    }
    ```
  - 将 `uint32_t` 数据转成 `uint8_t` 数组，按大端存储
    ```c
    void uint32_to_uint8_BigEnd(uint32_t src, uint8_t dest[4]) {
    dest[0] = (src >> 24) & 0xFF;
    dest[1] = (src >> 16) & 0xFF;
    dest[2] = (src >> 8) & 0xFF;
    dest[3] = (src) & 0xFF;
...
> 需要注意的是，拿到数组首地址之后并不能直接用`sizeof(pt)`来读取该指针指向的数组长度，这个时候用`sizeof()`，其会返回这个指针对应地址的数据长度，此时应该手动声明其数组的长度信息（这个`sizeof()`只适用于一开始就被定义好大小的数组。
...
- `static`关键字
  ...
  - 注意，由于`static`声明的变量只初始化一次，且生命周期贯穿程序执行全过程，所以一个文件内需要管理好静态变量名，**防止**栈区变量和`static`定义的静态变量**重名**。
- `extern`关键字
...
- `sizeof()`是在编译期确定数值的，对于确定大小的数组，比如：`int data[36] = {0x00};`，他会返回`36`,但是对于指向数组的指针，他无法返回其指向数组的长度，这个时候如果想拿到数组长度只能手动声明，用`sizeof()`会得到这个指针变量所指向地址的长度，一般是 4.

## 编程思路

- 对于大型项目来说，通常要分模块测试，建议开设一个`test.c`文件，用于提供不同模块的测试函数，`main.c`中调用这些测试函数的接口进行模块的测试，这样的测试效果是最好的。（不要像有人写了一堆奇奇怪怪的`main_*`函数）
  > 如果模块多的话，建议新建`test`文件夹下份多模块管理，提供各个模块的测试文件。
