...
我对 Linux 的了解不深，甚至可以说只知道`sudo`在命令行提权，其他都没有做很深入的了解，目前我电脑上搭载的是 win 和 kubuntu 的双系统，之前在安装正统 ubuntu 的时候出现了极大的困难（读取不到显卡导致屏幕亮度无法调暗，极其影响使用体验）最后在朋友的帮助下安装了`kubuntu`。但是由于之前只分配了 40G 的内存，现在 linux 存储告急，以扩容为契机，我准备系统学习一下 linux 操作系统，所以也就新增了这个版块。

2025-06，进入博士生涯，在自己台式机上重新安装的 Ubuntu，但是对存储的分配还是很迷惑，不过这次给的空间够大，好用的。这次对这个文档重新进行了整理，了解的更深入了
